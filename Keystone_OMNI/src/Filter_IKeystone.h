...
extern "C" {
    ...
    DECLARE_INTERFACE_(IKeystone, IUnknown)
    {
		...
		//IMPORTED FROM KEYHD
		STDMETHOD ( StartRecording ) (THIS_ 
            LPCOLESTR pszFileName
		);

		STDMETHOD ( StopRecording ) (THIS_ );

		STDMETHOD ( SetTrialOverride ) (THIS_ 
            bool bOverride
		);

		STDMETHOD ( SendMediaTimeEvents ) (THIS_ 
            int bActive,
			int iType
		);
		//IMPORTED FROM KEYHD

		STDMETHOD ( SetOptimizedLevel ) (THIS_ 
			int iOptimizedLevel
		);

		STDMETHOD ( SetRenderer ) (THIS_ 
			int iRenderer
		);
