...
STDMETHODIMP CKeystone::get_InputWidth(long *InputWidth)
{
	...
	{
		CMediaType* pmt = &m_pVideo->CurrentMediaType();
...
STDMETHODIMP CKeystone::FrameStep(bool bForward)
{
	...
	//Gabor's new way
	if (!m_pVideo->bFrameStepping)
	{
		m_pVideo->bFrameStepping = true;
	}
	else
	{
		m_pVideo->bDeliverFrameStepSample = true;
	}
	return S_OK;
}
...
{
	m_pVideo->ActivateVarispeed(Speed);
	return S_OK;
}

STDMETHODIMP CKeystone::DeactivateVarispeed()
{
	m_pVideo->DeactivateVarispeed();
	return S_OK;
}
...
STDMETHODIMP CKeystone::ShowBitmap(LPBYTE pBMP, int W, int H, int X, int Y, int Format)
{
	...
	GetBMPSample(pBMP, X, Y, W, H, &pSample1);
	...
	GetBMPSample(pBMP, X, Y, W, H, &pSample2);
...
{
	m_pVideo->bInFFRW = true;
	return S_OK;
}

STDMETHODIMP CKeystone::DeactivateFFRW()
{
	m_pVideo->bInFFRW = false;
	SetupVideoSizing();
	return S_OK;
}
...
{
	//return ResendSample();
	return ReSizeMixSendSample(Unconditional);
	//return S_OK;
}
...
STDMETHODIMP CKeystone::ConvertJacketPicture(LPCTSTR stJPPath, int W, int H, LPBYTE * out_pData)
{
	LPBYTE test	= (LPBYTE)GetJPSampleEx(stJPPath, 0, 0, W, H);
	//BYTE b = 1;
	//for (int i = 0; i < H*W*2; i++)
	//{
	//	b = test[i];
	//	//DbgLog((LOG_TRACE, 0, TEXT("byte: %d"), test[i]));
	//}
	
	LPBYTE Test2 = (LPBYTE)malloc(H*W*3);
	...
	CSCManual::UYVYToRGB24(test, W, H, Test2);
	*out_pData = Test2;
	return S_OK;
}
