...
CKeystone::CKeystone(TCHAR     *pName,
	...
	L21_X=40;
	L21_Y=0;
	L21_Initialized = false;
	opFrameCnt = 0;
	LB_OK = 1;
	PS_OK = 1;
	dwActionTitleSafeColor = 0x00992277;
	dwLastSampleProcessedAt = 0;
	bSetNULLTimestamps = false;

	//GOP TCs
	iMixGOPTC=0;
	iMixGOPTC_IFrameRed = 0;
	InitGOPTC(m_hDC, m_hBM, m_hFONT, 100, 16);

	LastFrameOutput = (LPBYTE)malloc(829440);
	LastFrameReceived = (LPBYTE)malloc(829440);
	LastFrameReceived_Resized = (LPBYTE)malloc(829440);
...
CKeystone::~CKeystone()
{
	//ULONG refcnt = m_pVideo->Release(); //doesn't work
	...
	free(LastFrameOutput);
	free(LastFrameReceived);
	free(LastFrameReceived_Resized);
}
...
HRESULT CKeystone::CheckTransform(const CMediaType* mtIn, const CMediaType* mtOut)
{
	...
	//VIDEOINFOHEADER2 * VIH2 = (VIDEOINFOHEADER2*) mtOut->Format();
...
STDMETHODIMP CKeystone::Run(REFERENCE_TIME tStart)
{
	...
	if (m_mtIn.formattype == FORMAT_VideoInfo2)
	{
		VIH2In = (VIDEOINFOHEADER2*)m_mtIn.pbFormat;
		InIsVIH2 = true;
	...
	{
		VIHIn = (VIDEOINFOHEADER*)m_mtIn.pbFormat;
		InIsVIH2 = false;
