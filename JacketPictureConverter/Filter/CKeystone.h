...
public:
	...
	HRESULT MixSubpicture(LPBYTE dst, int w, int h);
	HRESULT MixOnScreenDisplay(LPBYTE dst, int w, int h);
	HRESULT MixLine21(LPBYTE dst, int w, int h);
	HRESULT MixGuides(LPBYTE dst, int w, int h);
	HRESULT MixGOPTimecode(LPBYTE dst, IMediaSample * pOut, int w, int h);
	...
	void BurnInTicks(LPBYTE dst, int w, int h);
...
public:
	...
	HRESULT GetJPSampleEx(LPCTSTR fn, int x, int y, int w, int h);
	...
	HRESULT ReSizeMixSendSample(int Unconditional);
	...
	LPBYTE LastFrameReceived;
	LPBYTE LastFrameReceived_Resized;
	//HRESULT GetSampleForLastBuffer(LPBYTE pBMP, IMediaSample** ppOutSample);
...
public:
	...
	STDMETHODIMP ConvertJacketPicture(LPCTSTR stJPPath, int W, int H, LPBYTE * out_pData);
