...
void CKeystone::BurnInTicks(LPBYTE dst, int w, int h)
{
	TCHAR buff[256];
	//_stprintf_s(buff, sizeof(buff)/sizeof(buff[0]), _T("%02d:%02d:%02d.%02d\n"), h, m, s, f);
	...
	SetBkColor(m_hDC, 0x00ff00);
	SetTextColor(m_hDC, 0x000000);

	HGDIOBJ hBMOld = SelectObject(m_hDC, m_hBM);
	...
	SIZE size;
	::GetTextExtentPoint32(m_hDC, buff, _tcslen(buff), &size);

	TextOut(m_hDC, (bm.bmWidth - size.cx) / 2, (abs(bm.bmHeight) - size.cy) / 2, buff, _tcslen(buff));
