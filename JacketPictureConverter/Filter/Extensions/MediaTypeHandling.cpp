...
bool CKeystone::CheckForMTChange(IMediaSample *pIn)
{
    ...
    if (S_OK == pIn->GetMediaType((AM_MEDIA_TYPE**)&mSource) && mSource)
	{
		...
		if (VIH2In->bmiHeader.biHeight != LastConnectedHeight_VidIn)
		{
			...
			m_pVideo->ZoomDeInit();
			m_pVideo->ZoomInit(720, VIH2In->bmiHeader.biHeight);
...
HRESULT CKeystone::SetupVideoSizing()
{
	...
	DbgLog((LOG_TRACE, 0, TEXT("KEYSTONE: NewAR. DesResizeMode: %d  ResizeMode: %d  dwCF: %d  X: %d  Y: %d  L: %d  R: %d"), m_pVideo->DesResizeMode, m_pVideo->ResizeMode, dwCF, X, Y, VIH2In->rcSource.left, VIH2In->rcSource.right));

	if (VIH2In->rcSource.left == 90) X = 16;

    switch(m_pVideo->DesResizeMode)
    {
        case 1: //Anamorphic
        {
			m_pVideo->ResizeMode = 1;
...
CaseLetterbox:
	...
	//		if (m_pVideo->DesResizeMode == 1)
	//		{
	//		//User has selected anamorphic
	//			m_pVideo->ResizeMode = 1;
	//			DbgLog((LOG_TRACE, 0, TEXT("KEYSTONE: OUTPUT - no change")));
	//		}
	//		else if (m_pVideo->DesResizeMode == 2)
	//		{
	//		//User has selected panscan
	//			m_pVideo->ResizeMode = 2;
	//			DbgLog((LOG_TRACE, 0, TEXT("KEYSTONE: OUTPUT - panscanned")));
	//		}
	//		else if (m_pVideo->DesResizeMode == 3)
	//		{
	...
	//m_pVideo->ResizeMode = 3; //LETTERBOX
	//if ((VIH2In->rcSource.left == 90) & (VIH2In->rcSource.right == 630))
