		...
		case STATE_END:
			{
				...
				if(picture && fbuf && !(picture->flags&PIC_FLAG_SKIP))
				{
					...
					if(SUCCEEDED(hr))
					{
						...
						SetFullBufferBGColorYUY2(dst, 1440, w, dsth);
...
HRESULT CKeystone::GetJPSampleEx(LPCTSTR fn, int x, int y, int w, int h)
{
	//ASSERT(*ppOutSample == NULL);
		...
		case STATE_END:
			{
				...
				if(picture && fbuf && !(picture->flags&PIC_FLAG_SKIP))
				{
					...
					if(SUCCEEDED(hr))
					{
						...
						LPBYTE dst = (LPBYTE)malloc(w*h*2);
						//hr = pSample->GetPointer(&dst);
						...
						//SetFullBufferBGColorYUY2(ppOutSample, w*2, w, dsth);
