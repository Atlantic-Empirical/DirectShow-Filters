...
{
	m_pVideoOne->ResizeMode = Mode;
	m_pVideoOne->DesResizeMode = Mode;
	return S_OK;
}

STDMETHODIMP CKeystone::SetLBColor(int Red, int Green, int Blue)
{
	m_pVideoOne->dwLBColor = CSCManual::RGBToYUV(Blue, Green, Red);
	return S_OK;
}
...
{
	m_pVideoOne->bForceReverseFieldOrder = bReverseIt;
	return S_OK;
}

STDMETHODIMP CKeystone::SetJacketPicMode(bool bJackPicMode)
{
	m_pVideoOne->bInJacketPictureMode = bJackPicMode;
	return S_OK;
}
...
STDMETHODIMP CKeystone::BumpFieldsDown(int bBumpFields)
{
	if (bBumpFields == 0)
	{
		m_pVideoOne->bForceBumpFieldsDown = false;
	}
	else
	{
		m_pVideoOne->bForceBumpFieldsDown = true;
	}
	return S_OK;
}
...
{
	m_pVideoOne->DoFieldSplitting = DoFieldSplit;
	return S_OK;
}
...
STDMETHODIMP CKeystone::SetFrameSplit(int nAnchorX, int nAnchorY, int nR1_X, int nR1_Y, int nR2_X, int nR2_Y)
{
	DbgLog((LOG_TRACE, 0, TEXT("KEYSTONE: SetFrameSplit: %d %d %d %d %d %d"), nAnchorX, nAnchorY, nR1_X, nR1_Y, nR2_X, nR2_Y));
	AnchorX = nAnchorX;
	AnchorY = nAnchorY;
	R1_X = nR1_X;
	R1_Y = nR1_Y;
