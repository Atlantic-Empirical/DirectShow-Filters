...
#include "Pins/VideoOne/VideoOnePin.h"
#include "Pins/VideoTwo/VideoTwoPin.h"
#include "Pins/Output/OutputPin.h"
#include "vmr9.h"
#include "../VideoProcessing/ProcAmp/ProcAmp.h"
#include "../VideoProcessing/ColorFilters/ColorFilters.h"
#include <atlbase.h>
...
public:
    ...
    friend class CKeystoneVideoOnePin;
    friend class CKeystoneVideoTwoPin;
    friend class CKeystoneOutputPin;

	CKeystoneVideoOnePin *m_pVideoOne;
    CKeystoneVideoTwoPin *m_pVideoTwo;
    ...
    HRESULT ReceiveVideoTwo(LPBYTE EncodeBuffer);
    ...
    // Reveals IKeystone & ISpecifyPropertyPages
...
public:
	...
	//Gabor Jacket Picture Code
	HRESULT GetJPSample(LPCTSTR fn, int x, int y, int w, int h, IMediaSample** ppOutSample);

	//Bitmap rendering
	HRESULT GetBMPSample(LPBYTE pBMP, int x, int y, int w, int h, IMediaSample** ppOutSample);
	...
	// Fake frame number used to recalculate timestamps.
	UINT m_iFrame;
...
#define	QUEUE_ENTS	(16)	//32)			// must be a power of two
#define QUEUE_MASK	(QUEUE_ENTS-1)
#define FRAME_SIZE	(4147200)

typedef	BYTE	MYBUF[FRAME_SIZE];

	BOOL m_bPinTwoIsWaiting;
	HANDLE m_hEvent;
	...
	BYTE VTM_Current;
	BYTE VTM_LastStored;
	BYTE VTM_HasBeenMixed;
	BYTE VTM_HaveData;
	BYTE VTM_Initialized;
	LPBYTE EncodeBuffer;
	...
	void TextToBMP(HDC hDC, HBITMAP hBM, HFONT hFont, TCHAR buff[256], double fps, DWORD fgcolor = 0xffffff, DWORD bgcolor = 0x000000);
...
public:
	...
	// IKeystone
    STDMETHODIMP get_InputOneMediaType(CMediaType *InputMediaType);
    STDMETHODIMP get_InputTwoMediaType(CMediaType *InputMediaType);
	...
	STDMETHODIMP SetFrameSplit(int nAnchorX, int nAnchorY, int nR1_X, int nR1_Y, int nR2_X, int nR2_Y);
