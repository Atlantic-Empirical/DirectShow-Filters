...
void CKeystoneVideoOnePin::ZoomDeInit()
{
	for(int i = 0; i < 3; i++)
	{
		delete [] s_yuv[i];
...
bool CKeystoneVideoOnePin::ZoomInit(int w, int h)
{
	ZoomDeInit();
	if(w&3) return false;
	s_yuvsize.cx = w;
...
void CKeystoneVideoOnePin::Resize1D(BYTE* d, int dw, BYTE* s, int sw, int step)
{
	if(step == 1)
	{
...
bool CKeystoneVideoOnePin::ZoomYUY2(LPBYTE src, int w, int h, RECT r)
{
