...
CKeystoneOutputPin::NonDelegatingQueryInterface(REFIID riid, void **ppv)
{
    ...
    *ppv = NULL;
    ...
    if (riid == IID_IMediaPosition || riid == IID_IMediaSeeking) 
	{
        ...
        ASSERT(m_pTransformFilter->m_pVideoOne != NULL);
        ...
        if (m_pPosition == NULL) {
            ...
            HRESULT hr = CreatePosPassThru(
                             GetOwner(),
                             FALSE,
                             (IPin *)m_pTransformFilter->m_pVideoOne,
                             &m_pPosition);
            if (FAILED(hr)) 
			{
                return hr;
...
CKeystoneOutputPin::CheckConnect(IPin *pPin)
{
    ...
    //ASSERT(m_pTransformFilter->m_pVideoOne != NULL);
    //if ((m_pTransformFilter->m_pVideoOne->IsConnected() == FALSE)) {
	   // return E_UNEXPECTED;
    //}
...
CKeystoneOutputPin::CheckMediaType(const CMediaType* pmtOut)
{
	...
	//return S_OK;
    return m_pTransformFilter->CheckTransform(&m_pTransformFilter->m_pVideoOne->CurrentMediaType(), pmtOut);
}
...
CKeystoneOutputPin::SetMediaType(const CMediaType* pmtOut)
{
    HRESULT hr = NOERROR;
    ASSERT(m_pTransformFilter->m_pVideoOne != NULL);
    //ASSERT(m_pTransformFilter->m_pVideoOne->CurrentMediaType().IsValid());
 ...
 //       
    ...
    if (FAILED(m_pTransformFilter->CheckTransform(&m_pTransformFilter->
					m_pVideoOne->CurrentMediaType(),pmtOut))) {
...
CKeystoneOutputPin::Notify(IBaseFilter * pSender, Quality q)
{
	...
	hr = m_pTransformFilter->m_pVideoOne->PassNotify(q);
	hr = m_pTransformFilter->m_pVideoTwo->PassNotify(q);
...
HRESULT CKeystoneOutputPin::AlterQuality(Quality q)
{
	...
	//DbgLog((LOG_TRACE, 0, TEXT("Keystone Quality Message. Type: %s Proportion: %d Late: %I64d TimeStamp: %I64d"), Type, q.Proportion, q.Late, q.TimeStamp));
	//m_pTransformFilter->NotifyEvent(EC_QUALITY_CHANGE_KEYSTONE, q.Proportion, (LONG_PTR)q.Late);
	UNREFERENCED_PARAMETER(q);
    return S_OK; //do not pass qual msg upstram
    //return S_FALSE;
}
...
HRESULT CKeystoneOutputPin::GetMediaType(int iPosition, CMediaType* pmt)
{
	...
	//if ((iPosition == 0) && m_pTransformFilter->m_pVideoOne->IsConnected())
	//{
	//	int h,w;
	//	REFERENCE_TIME ft;
	//	if (m_pTransformFilter->m_pVideoOne->CurrentMediaType().formattype == FORMAT_VideoInfo2)
	//	{
	//		VIDEOINFOHEADER2* inType = (VIDEOINFOHEADER2*)m_pTransformFilter->m_pVideoOne->CurrentMediaType().Format();
	//		h = inType->bmiHeader.biHeight;
	//		w = inType->bmiHeader.biWidth;
	//	}
	//	else
	//	{
	//		VIDEOINFOHEADER* inType = (VIDEOINFOHEADER*)m_pTransformFilter->m_pVideoOne->CurrentMediaType().Format();
	//		h = inType->bmiHeader.biHeight;
	//		w = inType->bmiHeader.biWidth;
	//	}
	...
	if (iPosition == 0)
	{
		//return CreateVideoMediaType(720, 486, 16, D3DFMT_UYVY, 417084, pmt);
		return CreateVideoMediaType(1920, 1080, 16, D3DFMT_UYVY, 417084, pmt);
	}
	else
	{
		return hr;
...
// Just for debugging.
HRESULT CKeystoneOutputPin::Inactive()
{
	HRESULT hr = 2;
	//_asm {
	//	int 3
