...
//	}
	...
	static HRESULT MixUYVY_VideoTwo(LPBYTE src, LPBYTE dst, int AnchorX, int AnchorY, int R1_X, int R1_Y, int R2_X, int R2_Y)
	{ 
		////TODO: Make this use the radials
		//int h = min(srch, dsth);
		//int srcpitch = srcw*2;
		//int dstpitch = dstw*2;

		//for(int j = 0; j < dsth; j++, src += srcpitch, dst += dstpitch) 
		//{ 
		//	WORD* s = (WORD*) src; 
		//	WORD* d = (WORD*) dst; 
		//	for(int i = 0; i < dstw; i += 2, s += 2, d += 2) 
		//	{ 
		//		BYTE u = d[0]&0xff; 
		//		BYTE y1 = d[0]>>8; 
		//		BYTE v = d[1]&0xff; 
	...
	static HRESULT MixImage(LPBYTE TargetImage, int Targ_W, int Targ_H, int Targ_Pitch, int Targ_Format, CKeystone::stMixData MixMe)
	{
		if (Targ_Format != MixMe.nImage_Format)
		{
			LPBYTE ConversionTarget = NULL;
			//Convert MixImage to Targ_Format
			if (Targ_Format == CKeystone::IF_RGB24)
			{
				if (MixMe.nImage_Format == CKeystone::IF_RGB32)
				{
					CSCManual::RGB32ToRGB24(MixMe.pImage, MixMe.nW, MixMe.nH, ConversionTarget);
				}
				if (MixMe.nImage_Format == CKeystone::IF_UYVY)
				{
 					CSCManual::UYVYToRGB24(MixMe.pImage, MixMe.nW, MixMe.nH, ConversionTarget);
				}
				if (MixMe.nImage_Format == CKeystone::IF_YUY2)
				{
					CSCManual::YUY2ToRGB24(MixMe.pImage, MixMe.nW, MixMe.nH, ConversionTarget);
				}
				if (MixMe.nImage_Format == CKeystone::IF_ARGB4444)
				{
					CSCManual::ARGB4444ToRGB24(MixMe.pImage, MixMe.nW, MixMe.nH, ConversionTarget);
				}
			}

			if (Targ_Format == CKeystone::IF_RGB32)
			{
				if (MixMe.nImage_Format == CKeystone::IF_RGB24)
				{
			...
			if (Targ_Format == CKeystone::IF_UYVY)
			{
				if (MixMe.nImage_Format == CKeystone::IF_RGB24)
				{
			...
			if (Targ_Format == CKeystone::IF_YUY2)
			{
				if (MixMe.nImage_Format == CKeystone::IF_RGB24)
				{
			...
			if (Targ_Format == CKeystone::IF_ARGB4444)
			{
				if (MixMe.nImage_Format == CKeystone::IF_RGB24)
				{
